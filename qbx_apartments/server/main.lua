...
-- Events
lib.addCommand('apartment', {
    help = 'Teleport to your apartment',
    restricted = false
}, function(source)
    TriggerClientEvent('apartments:client:LastLocationHouse', source)
end)

RegisterNetEvent('apartments:server:SetInsideMeta', function(house, insideId, bool, isVisiting)
    local src = source
...
RegisterNetEvent('apartments:server:OpenDoor', function(target, apartmentId, apartment)
    ...
    local ownerPlayer = exports.qbx_core:GetPlayer(source)
    if otherPlayer then
